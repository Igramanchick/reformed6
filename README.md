...
| `images/care-works.jpg` | `care-works.html` | 16:9. **Real photograph, in place.** Free-license stock photo from Pexels (photographer: RDNE Stock project), showing four generations of a family &mdash; a young woman, a child, a grandmother and a mother in her 30s&ndash;40s &mdash; joining hands together on a sofa, chosen to reflect the "sandwich generation" households Care Works and Caring for Caregivers actually serve. Its `alt` describes the scene; the reference carries `?v=3`, same cache-bust note as above (bumped from the prior photo). |
